...
Functions:
  ...
  all_callables()-- callables of this and inherited interfaces."""
    ...
    def _setContents(self, contents):
        ...
        self.__declarations = [c for c in contents if not (isinstance(c, Attribute) or
                                                    isinstance(c, Operation))]
        self.__callables    = [c for c in contents if (isinstance(c, Attribute) or
                                                isinstance(c, Operation))]
...
Functions:
  ...
  """
    ...
    def _setContents(self, contents):
        self.__contents     = contents
        self.__declarations = [c for c in contents if not \
        ...
        self.__statemembers = [c for c in contents if (isinstance(c, StateMember))]

        self.__factories    = [c for c in contents if (isinstance(c, Factory))]
