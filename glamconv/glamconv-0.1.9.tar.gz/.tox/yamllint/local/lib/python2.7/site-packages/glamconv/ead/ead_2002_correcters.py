...
class ChangeChildrenOrderer(TransformAction):
    ...
    desc = (
        u"In EAD, the children of <change> must occur in a given order "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            ...
            if log_details:
                logger.info(
                    u"The following elements have been corrected:", u"\n".join(log_data)
                )
        return xml_root
...
class EadheaderChildrenOrderer(TransformAction):
    ...
    category = u"EAD 2002 correction"
    desc = (
        u"In EAD, the children of <eadheader> must occur in a given order "
...
class ArchdescChildrenOrderer(TransformAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for elt in xml_root.xpath(xpath_req):
            ...
            dsc = elt.find(u"dsc")
            for subelt in elt.xpath(u"*[not(self::dsc) and preceding-sibling::dsc]"):
...
class CChildrenOrderer(TransformAction):
    ...
    category = u"EAD 2002 correction"
    desc = (
        u"In EAD, the children of <c> must occur in a given order (first "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for elt in xml_root.xpath(xpath_req):
            ...
            c = elt.find(u"c")
            for subelt in elt.xpath(u"*[not(self::c) and preceding-sibling::c]"):
...
class EmptyEltsEraser(TransformAction):
    ...
    category = u"EAD 2002 correction"
    desc = (
        u"In EAD, numerous elements must have at least one child. This "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if log_details:
            log_data = []
        xpath_req = u" | ".join(
            [u".//{0}[not(*)]".format(eltname) for eltname in EMPTY_ELTS_TO_ERASE]
        )
        for elt in xml_root.xpath(xpath_req):
            count += 1
            if log_details:
        ...
        if count > 0:
            logger.warning(u"{0:d} empty elements have been deleted." u"".format(count))
            if log_details:
                logger.warning(
                    u"The following empty elements have been" u"deleted:",
                    u"\n".join(log_data),
                )
        return xml_root
...
class ParagraphAdder(TransformAction):
    ...
    category = u"EAD 2002 correction"
    desc = (
        u"In EAD, numerous elements must have at least one child element "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            logger.warning(
                u"In {0:d} elements, an empty paragraph has been "
                u'added in order to have at least one "content" '
                u"child.".format(count)
            )
            if log_details:
                logger.warning(
                    u"The following elements have been modified:", u"\n".join(log_data)
                )
        return xml_root
...
class EmptyAttrsEraser(TransformAction):
    ...
    category = u"EAD 2002 correction"
    desc = (
        u"In EAD, some attributes must have a value taken from a "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for eltname, attnames in EMPTY_ATTRS_TO_ERASE.items():
            xpath_pred = u" or ".join(
                [
                    u'@{0}[normalize-space(text())=""]' u"".format(name)
        ...
        if count_elts > 0:
            logger.warning(
                u"{0:d} empty attributes have been deleted from "
                u"{1:d} elements.".format(count_atts, count_elts)
            )
            if log_details:
                logger.warning(
                    u"The following elements have had some " u"attributes deleted:",
                    u"\n".join(log_data),
                )
        return xml_root
...
class IdentifierConverter(TransformAction):
    ...
    category = u"EAD 2002 correction"
    desc = (
        u"In EAD, except in <eadid> and <unitid>, the identifier must be "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count1 > 0:
            logger.warning(
                u"{0:d} elements have had an 'id' attribute "
                u"defined from their 'identifier' attribute."
                u"".format(count1)
            )
            if log_details:
                logger.warning(
                    u"The following elements have been corrected:",
                    u"\n".join(log_data1),
                )
        if count2 > 0:
            logger.warning(
                u"{0:d} elements have had their 'identifier' "
        ...
        if log_details:
            log_data1 = []
            log_data2 = []
        for elt in xml_root.xpath(u".//*[(self::unitid|self::eadid) and @id]"):
            if u"identifier" not in elt.attrib:
                elt.set(u"identifier", elt.attrib.pop(u"id"))
                count1 += 1
                if log_details:
                    log_data1.append(log_element(elt, attributes=(u"identifier",)))
            else:
                count2 += 1
                if log_details:
                    log_data2.append(
        ...
        if count2 > 0:
            logger.warning(
                u"{0:d} elements have had their 'id' attribute "
...
class UnitdateCertainlyConverter(TransformAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for elt in xml_root.xpath(u".//unitdate[@certainly]"):
            ...
            if log_details:
                log_data.append(
                    log_element(elt, attributes=(u"certainty",), text_content=True)
                )
        if count > 0:
            logger.warning(
                u"{0:d} elements have had a 'certainty' attribute "
...
class COTHERlevelConverter(TransformAction):
    ...
    category = u"EAD 2002 correction"
    desc = (
        u"In EAD, the attribute for describing the other level inside a "
...
class LowercaseElements(TransformAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            logger.warning(u"{0:d} elements got lowercased".format(count))
            if log_details:
                logger.warning(
                    u"The following elements have been corrected:", u"\n".join(log_data)
                )
...
class NormalizeIdAttributes(TransformAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for node in xml_root.xpath(".//*[@id]"):
            ...
            node.tag = node.tag.lower()
        if count > 0:
            logger.warning(u"{0:d} id attributes got normalized".format(count))
            if log_details:
                logger.warning(
                    u"The following elements have been corrected:", u"\n".join(log_data)
                )
